...
class EmageVQVAEConv(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.quantizer = Quantizer(
            config.vae_codebook_size, config.vae_length, config.vae_quantizer_lambda
        )
        self.decoder = VQDecoderV5(config)
...
class EmageVQModel(nn.Module):
    ...
    def spilt_inputs(
        self, smplx_body_rot6d, expression, tar_contact=None, tar_trans=None
    ):
        ...
        upper_rot6d = smplx_body_rot6d[:, :, self.joint_mask_upper, :].reshape(
            bs, t, 78
        )
        hands_rot6d = smplx_body_rot6d[:, :, 25:55, :].reshape(bs, t, 180)
        lower_rot6d = smplx_body_rot6d[:, :, self.joint_mask_lower, :].reshape(
            bs, t, 54
        )
        tar_contact = (
            torch.zeros(bs, t, 4, device=smplx_body_rot6d.device)
            if tar_contact is None
            else tar_contact
        )
        tar_trans = (
            torch.zeros(bs, t, 3, device=smplx_body_rot6d.device)
    ...
    def map2index(self, smplx_body_rot6d, expression, tar_contact=None, tar_trans=None):
        inputs = self.spilt_inputs(
            smplx_body_rot6d, expression, tar_contact=tar_contact, tar_trans=tar_trans
        )
        return dict(
    ...
    def decode(
        ...
        if upper_index is not None:
            ...
            upper = rotation_6d_to_axis_angle(upper_6d.reshape(bs, seq, -1, 6)).reshape(
                bs, seq, -1
            )
        ...
        if hands_index is not None:
            ...
            hands = rotation_6d_to_axis_angle(hands_6d.reshape(bs, seq, -1, 6)).reshape(
                bs, seq, -1
            )
        ...
        if lower_index is not None:
            ...
            lower = rotation_6d_to_axis_angle(lower_6d.reshape(bs, seq, -1, 6)).reshape(
                bs, seq, -1
            )
            ...
            lower_6d = axis_angle_to_rotation_6d(lower.reshape(bs, seq, -1, 3)).reshape(
                bs, seq, -1
            )
        ...
        all_motion_rot6d = axis_angle_to_rotation_6d(
            all_motion_axis_angle.reshape(bs, seq, 55, 3)
        ).reshape(bs, seq, 55 * 6)
    ...
    def _get_global_motion(self, lower_body, ref_trans):
        ...
        rec_x_trans = velocity2position(
            rec_trans_v_s[:, :, 0:1], 1 / 30, ref_trans[:, 0, 0:1]
        )
        rec_z_trans = velocity2position(
            rec_trans_v_s[:, :, 2:3], 1 / 30, ref_trans[:, 0, 2:3]
        )
...
class EmageAudioModel(PreTrainedModel):
    ...
    def __init__(self, config: EmageAudioConfig):
        ...
        self.speaker_embedding_body = nn.Embedding(
            self.cfg.speaker_dims, self.cfg.hidden_size
        )
        self.speaker_embedding_face = nn.Embedding(
            self.cfg.speaker_dims, self.cfg.hidden_size
        )
        self.mask_embedding = nn.Parameter(
            torch.zeros(1, 1, self.cfg.pose_dims + 3 + 4)
        )
        ...
        self.bodyhints_face = MLP(
            self.cfg.motion_f, self.cfg.hidden_size, self.cfg.motion_f
        )
        self.bodyhints_body = MLP(
            self.cfg.motion_f, self.cfg.hidden_size, self.cfg.motion_f
        )
        ...
        self.position_embeddings = PeriodicPositionalEncoding(
            ...
            period=self.cfg.pose_length,
            max_seq_len=self.cfg.pose_length,
        )
        ...
        self.audio_motion_cross_attn_layer = nn.TransformerDecoderLayer(
            d_model=self.cfg.hidden_size,
            nhead=4,
            dim_feedforward=self.cfg.hidden_size * 2,
        )
        self.audio_motion_cross_attn = nn.TransformerDecoder(
            self.audio_motion_cross_attn_layer, num_layers=8
        )
        self.motion2latent_upper = MLP(
            self.cfg.hidden_size, self.cfg.hidden_size, self.cfg.hidden_size
        )
        self.motion2latent_hands = MLP(
            self.cfg.hidden_size, self.cfg.hidden_size, self.cfg.hidden_size
        )
        self.motion2latent_lower = MLP(
            self.cfg.hidden_size, self.cfg.hidden_size, self.cfg.hidden_size
        )
        self.body_motion_decoder_upper = nn.TransformerDecoder(
            self.audio_motion_cross_attn_layer, num_layers=1
        )
        self.body_motion_decoder_hands = nn.TransformerDecoder(
            self.audio_motion_cross_attn_layer, num_layers=1
        )
        self.body_motion_decoder_lower = nn.TransformerDecoder(
            self.audio_motion_cross_attn_layer, num_layers=1
        )
        self.motion_out_proj_upper = nn.Linear(
            self.cfg.hidden_size, self.cfg.vae_codebook_size
        )
        self.motion_out_proj_hands = nn.Linear(
            self.cfg.hidden_size, self.cfg.vae_codebook_size
        )
        self.motion_out_proj_lower = nn.Linear(
            self.cfg.hidden_size, self.cfg.vae_codebook_size
        )
        self.motion_cls_upper = MLP(
            self.cfg.vae_codebook_size, self.cfg.hidden_size, self.cfg.vae_codebook_size
        )
        self.motion_cls_hands = MLP(
            self.cfg.vae_codebook_size, self.cfg.hidden_size, self.cfg.vae_codebook_size
        )
        self.motion_cls_lower = MLP(
            self.cfg.vae_codebook_size, self.cfg.hidden_size, self.cfg.vae_codebook_size
        )
        self.audio_face_motion_proj = nn.Linear(
            self.cfg.audio_f + self.cfg.motion_f, self.cfg.hidden_size
        )
        self.face_motion_decoder = nn.TransformerDecoder(
            self.audio_motion_cross_attn_layer, num_layers=4
        )
    ...
    def forward(self, audio, speaker_id, masked_motion, mask, use_audio=True):
        ...
        speaker_motion_fea_proj = self.speaker_embedding_body(speaker_id).repeat(
            1, t, 1
        )
        speaker_face_fea_proj = self.speaker_embedding_face(speaker_id).repeat(1, t, 1)

        audio2face_fea_proj = self.audio_face_motion_proj(
            torch.cat([audio2face_fea, body_hint_face], dim=2)
        )
        face_proj = self.position_embeddings(speaker_face_fea_proj)
        decode_face = self.face_motion_decoder(
            tgt=face_proj.permute(1, 0, 2), memory=audio2face_fea_proj.permute(1, 0, 2)
        ).permute(1, 0, 2)
        ...
        motion_fea = self.motion_self_encoder(
            masked_motion_proj.permute(1, 0, 2)
        ).permute(1, 0, 2)
        ...
        audio2body_fea_cross = self.audio_motion_cross_attn(
            tgt=motion_fea.permute(1, 0, 2), memory=audio2body_fea_proj.permute(1, 0, 2)
        ).permute(1, 0, 2)
        if not use_audio:
        ...
        motion_upper_refine = self.body_motion_decoder_upper(
            tgt=upper_latent.permute(1, 0, 2)
            + speaker_motion_fea_proj.permute(1, 0, 2),
            memory=(hands_latent + lower_latent).permute(1, 0, 2),
        ).permute(1, 0, 2)
        motion_hands_refine = self.body_motion_decoder_hands(
            tgt=hands_latent.permute(1, 0, 2)
            + speaker_motion_fea_proj.permute(1, 0, 2),
            memory=(upper_latent + lower_latent).permute(1, 0, 2),
        ).permute(1, 0, 2)
        motion_lower_refine = self.body_motion_decoder_lower(
            tgt=lower_latent.permute(1, 0, 2)
            + speaker_motion_fea_proj.permute(1, 0, 2),
            memory=(upper_latent + hands_latent).permute(1, 0, 2),
        ).permute(1, 0, 2)
    ...
    def inference(self, audio, speaker_id, vq_model, masked_motion=None, mask=None):
        ...
        for i in range(rounds):
            ...
            audio_slice = audio[
                :,
                start_idx * (16000 // 30) : start_idx * (16000 // 30) + audio_slice_len,
            ]
            net_out_val = self.forward(
                audio_slice,
                speaker_id,
            ...
            _, cls_face = torch.max(
                F.log_softmax(net_out_val["cls_face"], dim=2), dim=2
            )
            _, cls_upper = torch.max(
                F.log_softmax(net_out_val["cls_upper"], dim=2), dim=2
            )
            _, cls_hands = torch.max(
                F.log_softmax(net_out_val["cls_hands"], dim=2), dim=2
            )
            _, cls_lower = torch.max(
                F.log_softmax(net_out_val["cls_lower"], dim=2), dim=2
            )

            face_latent = (
                net_out_val["rec_face"]
                if self.cfg.lf > 0 and self.cfg.cf == 0
                else None
            )
            upper_latent = (
                net_out_val["rec_upper"]
                if self.cfg.lu > 0 and self.cfg.cu == 0
                else None
            )
            hands_latent = (
                net_out_val["rec_hands"]
                if self.cfg.lh > 0 and self.cfg.ch == 0
                else None
            )
            lower_latent = (
                net_out_val["rec_lower"]
                if self.cfg.ll > 0 and self.cfg.cl == 0
                else None
            )
        ...
        if remain > pre_frames:
            ...
            audio_slice = audio[
                :,
                final_start * (16000 // 30) : final_start * (16000 // 30)
                + audio_slice_len,
            ]
            net_out_val = self.forward(
                audio_slice,
                speaker_id,
                masked_motion=final_motion,
                mask=final_mask,
                use_audio=True,
            )
